...
Once your site is upgraded to the paid plan, you can configure your site to be served on a custom domain. Aerobatic will provision a wildcard SSL certificate for your domain using the [AWS Certificate Manager](https://aws.amazon.com/certificate-manager/). Wildcard certificates cover your apex, aka "naked", domain and all subdomains. This allows you to cover all your [deploy stages](/docs/deployment#deploy-stages) using the same certificate, for example:
...
<Cli>aero domain --name mydomain.com --subdomain www</Cli>

Or if you want to run your site at the apex (i.e. "naked domain"), then specify "@" as the sub-domain:

<Cli>aero domain --name mydomain.com --subdomain @</Cli>

See the [aero domain](/docs/cli#domain-command) command for more details.
...
| Record  | Name                                | Value                                                   |
| ------- | ----------------------------------- | ------------------------------------------------------- |
| `CNAME` | `_3639ac514e785e898d2646601fa951d5` | `_fbc6a92e9dedgh546e4b606a613d305.acm-validations.aws.` |

After you've created the record above, you can validate that it has propaaged by running the following command (substituting in the name of your validation CNAME):

<Cli>nslookup -q=cname _3639ac514e785e898d2646601fa951d5.mydomain.com</Cli>

<Alert type="warn" style={{marginTop: '25px'}}>

If your DNS provider prohibits a leading underscore in CNAME values, you can remove the leading underscore from the `_xxx.acm-validation.aws.` value.

So instead of: `_fbc6a92e9dedgh546e4b606a613d305.acm-validations.aws.`

Instead use: `fbc6a92e9dedgh546e4b606a613d305.acm-validations.aws.`.
...
If your DNS provider does not support `ANAME` or `ALIAS` records, we suggest transferring your nameservers to Cloudflare's free DNS hosting service and using CNAME flattening. See the [Cloudflare section](#cloudflare) below for details.

However if it's not possible to switch, then your best bet is to redirect the apex URL to a CNAME like `www`. Below are two possible ways to configure this:
...
<MdxImage
  screenshot
  src="https://www.aerobatic.com/media/docs/--1/cloudflare-dns-records.png"
  alt="Cloudflare DNS records"
/>

<Alert type="warn" style={{marginTop: '20px'}}>

**IMPORTANT** Make sure the cloud icon in the **Status** column for the validation CNAME is gray rather than orange. Otherwise the CNAME will **not** be recognized by the domain validation process.
...
| ----- | ------------------ | ----- | ------------------------------------ |
|       | `A - IPv4 address` | Yes   | `[domain_dns_value].cloudfront.net.` |
