...
import { Alert, Cli, Image, Option } from '$components/mdx';
...
<Image
  src="https://www.aerobatic.com/media/docs/search-results-basic-screenshots.png"
  alt="Basic Search Template Screenshot"
/>
...
<Option name="noIndexPatterns">The path patterns that should not be indexed.</Option>

<Option name="siteMapPath">

Override the path where the sitemap is located. Defaults to `/sitemap.xml`.

</Option>

<Option name="contentSelectors">

The list of DOM selector strings that identify the blocks of content that should be indexed. Defaults to `[p, li]`. See [Controlling what is indexed](#controlling-what-is-indexed).

</Option>

<Option name="headerSelectors">

The list of jQuery style selector strings that identify the content headers indexed. Defaults to `[h1, h2]`.

</Option>

<Option name="titleStripStart">

String that should be stripped off the start of the `<title>`. See [Page title formatting](#page-title-formatting).

</Option>

<Option name="titleStripEnd">

String that should be stripped off the end of the `<title>`.

</Option>

<Option name="metadataKeys">
...
<Cli>curl -H 'Accept:application/json' https://www.aerobatic.com/search?q=jekyll"</Cli>
...
<body>
  ...
  <div class="search-results">
    ...
    <div class="search-result">
      ...
      <cite>{{fullUrl}}</cite> {{#snippet}}
      <span class="snippet">{{#date}}{{date}} - {{/date}}{{{snippet}}}</span> {{/snippet}}
    </div>
    {{/searchResults}} {{#zeroResults}}
    <h4>No results</h4>
    {{/zeroResults}}
  </div>
</body>
...
Note the triple `{{{}}}` delimiters around the `snippet` and `urlPath`. This is how Mustache unescapes the output. This is necessary for the snippet since it contains `<em>` tags highligting keyword matches. When using custom Mustache tags you can unescape with syntax like this: `<%= &snippet %>`.
