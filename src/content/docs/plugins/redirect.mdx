...
The `.html` extension is stripped off automatically for you, so no need to specify a redirect for that.
...
<Alert type="warn">Make sure to use a double backslash to escape the question mark.</Alert>
