...
import { Option } from '$components/mdx';
...
<Option name="accessKeyId">
  The AWS access key of the IAM user to connect to S3 with (environment variable recommended).
</Option>

<Option name="secretAccessKey">
  The AWS secret access key (environment variable recommended).
</Option>

<Option name="region">The AWS region of the bucket, i.e. “us-west-2”.</Option>

<Option name="bucket">The name of the S3 bucket.</Option>

<Option name="prefix">
  Optional path to the root S3 folder where the files to be hosted live. If omitted, the http
  requests to the proxy need to mirror the full S3 path.
</Option>

<Option name="defaultCacheControl">
  Value of the `Cache-Control` header to use if the metadata from the S3 object does not specify
  it’s own value.
</Option>

<Option name="overrideCacheControl">
  Value of the Cache-Control header that is applied to the response even if there there is a
  different value on the S3 object metadata.
</Option>

### Creating the AWS IAM user {#create-iam-user}
...
### S3 transfer charges {#transfer-charges}
