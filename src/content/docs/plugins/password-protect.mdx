...
```html
<form method="POST">
  <input type="password" name="aerobatic-password" /> <button type="submit">Log in</button>
</form>
```
...
```html
<form method="GET">
  <input type="hidden" name="__logout" value="1" /> <button type="submit">Log out</button>
</form>
```
