...
<html>
  ...
  <body>
    <h1>My D3.js Demo</h1>
    <div id="d3div" class="d3"><script src="js/analysis.js"></script></div>
  </body>
</html>
```

## Delcare plugin in aerobatic.yml

The [s3-proxy](/docs/plugins/s3-proxy/) plugin needs to be declared in the [aerobatic.yml](/docs/configuration/) like so:
...
d3.csv(
  '/s3-proxy/data/adWordsLocations.csv',
  function(d) {
    return {
      criteria_id: +d['Criteria ID'],
      name: d.Name,
  ...
  function(data) {
    ...
    x.domain(
      countryCount.map(function(d) {
        return d.key;
      })
    );
    y.domain([
      0,
      d3.max(countryCount, function(d) {
        return d.values;
      })
    ]);
