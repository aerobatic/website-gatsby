...
<MdxImage src="/img/search-scan-icon.png" style={{ float: 'right', maxWidth: '200px' }} />
...
<Cli>curl -H 'Accept:application/json' https://hugo-search-demo.aerobaticapp.com/search/?q=raven"</Cli>
...
<MdxImage
  ...
  src="https://www.aerobatic.com/media/blog/hugo-search-demo-screenshot.png"
...
```html
<form class="search" method="get" action="/search/"><input type="text" name="q" placeholder="Search poems" /></form>
```
...
{{ if .Params.description }}<meta name="description" content="{{ .Params.description }}" />{{ end }} {{ if .Params.author }}<meta
  name="author"
  content="{{ .Params.author }}"
/>{{ end }} {{ if .Params.year }}<meta name="year" content="{{ .Params.year }}" />{{ end }}
```
